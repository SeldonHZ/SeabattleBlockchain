export default function main() {
	...
	class Field {
		...
		constructor(field) {
			...
			let coordinates = getCoordinates(this.field);
  			if (coordinates) {
		...
		createShip() {
			...
			if (player === human) {
				...
				if (Object.keys(player.squadron).length === 10) {
					let checkBug = player.matrix
					checkBug = checkBug.flat(Infinity);
					let sum = 0;
					for (let i = 0; i < checkBug.length; i++) { sum += checkBug[i]; }
					if (sum === 20) {
						buttonPlay.hidden = false;
					} else {
						buttonPlay.hidden = true;
						alert("Something seems wrong... Reload the page");
					}
				}
			}
		}
	...
	class Placement {
		...
		constructor() {
			if (correction === false) {
				Placement.FRAME_COORDS.top -= 33;
				Placement.FRAME_COORDS.bottom -= 33;
				correction = true;
			}
			this.dragObject = {};
		...
		getCoordsCloneInMatrix({left, right, top, bottom} = this.coords) {
	...
	const mintpnl = getElement('mintpnl');
	...
	mint.addEventListener('click', function(e) {
		mintpnl.hidden = true;
		let user = e.srcElement.value;
		let data = computer.matrix;
		let requestBody = [ data, user ];
